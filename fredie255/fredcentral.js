...
async function createFredcentralTable() {
  const client = await pool.connect();

  try {
    // Créez la table fredcentral si elle n'existe pas déjà
    await client.query(`
      CREATE TABLE IF NOT EXISTS fredcentral(
    ...
    `);
    console.log('Table fredcentral créée avec succès');
  } catch (error) {
    console.error('Erreur lors de la création de la table fredcentral:', error);
  } finally {
    client.release();
  }
}
...
async function insertData() {
  ...
  try {
    for (let i = 1; i <= 50; i++) {
      const query = `
        INSERT INTO fredcentral(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17, e18, e19, e20, e21, e22)
...
async function getData(ide) {
  ...
  try {
   const query = 'SELECT e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17, e18, e19, e20, e21, e22 FROM fredcentral WHERE id = $1';
