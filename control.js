...
function createNotification(deletedMessage) {
  ...
  let notification = `*🤦✧⁠LUCKY_MD✧ ANTIDELETE🤦*\n\n`;
...
if (conf.AUTO_REACT === "yes") {
  zk.ev.on("messages.upsert", async m => {
    ...
    try {
      ...
      console.error('Error reading like file:', error);
      return;
    }
...
if (conf.AUTO_REACT_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      if (! superUser && origineMessage === auteurMessage && conf.AUTO_BLOCK === 'yes') {
        zk.sendMessage(auteurMessage, {
          'text': "🚫am blocking you because you have violated Fredi policies🚫!"
        });
  ...
  if (!superUser) {
    return repondre("Only for my owner or frediezra to execute this command 🚫");
  }
    ...
    await zk.sendMessage(origineMessage, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          sourceUrl: conf.GURL,
          thumbnailUrl: "https://files.catbox.moe/7irwqn.jpeg" || conf.BOT_MENU_LINK,
          mediaType: 1,
        ...
        if (cd) {
          try {
            ...
            if (!superUser && origineMessage === auteurMessage && conf.PM_PERMIT === "yes") {
              repondre("SORRY!! ❌\n\nYou don't have acces to commands here my bro");
              return;
            }
            ///////////////////////////////
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        ...
        console.log("chargement des luckycmd ...\n");
