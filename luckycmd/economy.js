...
ezra({
    ...
    const { zerogroup, sck, eco, daily } = commandeOptions;
      try{
...
ezra({
    ...
    async (dest, zk, commandeOptions) => {
    const { zerogroup, sck, mongoschemas, eco, contextinfo, message } = commandeOptions;
      try{
...
async (dest, zk, commandeOptions) => {
const { zerogroup, sck, mongoschemas, eco, contextinfo, message, match, balance } = commandeOptions;
  try{
   let zerogroup = (await sck.findOne({ id: message.chat, })) || await sck.new({  id: message.chat,  })
   let mongoschemas = zerogroup.economy || "false";
   if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
...
await message.reply('*What are you trying to do📉*.')
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, contextinfo, message, deposit } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({ id: message.chat,  })) || {};
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
     //  let users = message.mentionedJid ? message.mentionedJid[0] : message.msg.contextInfo.participant || false;
       if (!match) return message.reply("Fredi!! Provide the 💰amount you want to deposit!");
       let d = parseInt(match)
       const deposit = await eco.deposit(message.sender, "Lucky", d);
   ...
   async (dest, zk, commandeOptions) => {
   const { sck1, eco, message } = commandeOptions;
     try{
   let h = await eco.lb("Lucky",10);
   let str = `*Top ${h.length} users with more money in wallet.*\n`
   const { sck1 } = require('../fred');
   let arr = []
...
const { zerogroup, sck, mongoschemas, eco, contextinfo, message, match, balance } = commandeOptions;
  try{
   ...
   if(!user) return message.reply('Please give me any user🤦‍♂️.');
       ...
       const balance = await eco.balance(user1, "Lucky");
       let a = (balance.wallet) < parseInt(word)
       //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
       if(a == true) return message.reply("you dont have sufficient money to transfer👎");

       const deduct = await eco.deduct(user1, "Lucky", value[0]);
       const give = await eco.give(user2, "Lucky", value[0]);
  ...
  // return await Aviator.bot.sendButtonText(message.chat, `*📠 Transaction successful of ${value[0]} 💰*`, `${Config.ownername.split(' ')[0]}-Economy Version: 0.0.6`, message);
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, message, balance } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({ id: message.chat,})) || await sck.new({id: message.chat,})
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(message.sender,"Lucky"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
   ...
   async (dest, zk, commandeOptions) => {
   const { eco, message, match } = commandeOptions;
     try{
       if(!message.isCreator) return message.reply(`*_Hey Master, only my owner can give money!_*`)
        ...
        await eco.give(users, "Lucky", parseInt(match.split(' ')[0]));
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, message, match, balance } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({  id: message.chat,  })) || (await sck.new({   id: message.chat,   }) );
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, contextinfo, message, match, balance, balance1, balance2 } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({   id: message.chat,  })) || (await sck.new({  id: message.chat,   }));
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
   ...
   if(!users) return message.reply('Please give me user to rob.')
       ...
       const user2 = users
       const k = 1000
       const balance1  = await eco.balance(user1, "Lucky")
   const balance2  = await eco.balance(user2, "Lucky")
   ...
   switch (random) {
     ...
     const deduff = Math.floor(Math.random() * 1000)	    
         await eco.deduct(user2, "Lucky", deduff);
         await eco.give(message.sender, "Lucky", deduff);
       ...
       case 'caught':
          ...
          await eco.deduct(user1, "Lucky", rmoney);
...
//message.react('🤔')
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, user, add, message, match } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({   id: message.chat,  })) || {} ;
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
   ...
   async (dest, zk, commandeOptions) => {
   const { zerogroup, sck, mongoschemas, eco, balance, message, match, media } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({ id: message.chat,})) || {};
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
...
///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
    const balance = await eco.balance(user, "Lucky");
  ...
  message.reply(media,{packname:"Lucky",author:'Economy'},"sticker")
    ...
    if (!match) return message.reply(`Example:  ${prefix}gamble 100 direction(left,right,up,down)`);
          ...
          if ( r == opp){
          let give = await eco.give(user , "Lucky", twice);    //message.react('⭐️')
   ...
   //return await Aviator.bot.sendButtonText(message.chat, `*📈 You won 🪙${twice}*`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, message);
                 ...
                 let deduct = await eco.deduct(user, "Lucky", texts[0]);
   ...
   async (dest, zk, commandeOptions) => {
  const { zerogroup, sck, mongoschemas, eco, balance1, message, match, deduff, balance, deduct, mess } = commandeOptions;
     try{
       let zerogroup = (await sck.findOne({  id: message.chat,    })) || {};
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
       ...
       if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
           ...
           const user = message.sender
           const k = 100
           const balance1  = await eco.balance(user,"Lucky")
...
if(value<=balance.wallet){
   ...
   if ((f1 !== f2) && f2 !== f3){
       const deduct1 = await eco.deduct(user, "Lucky", deduff);
       return message.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && f2 == f3){
       const give1 = await eco.give(user, "Lucky", deduff/2);
       return message.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
    }
    else if ((f1 == f2) && f2 !== f3){
       const give2 = await eco.give(user, "Lucky", deduff);
       return message.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f1 == f3){
       const deduct2 = await eco.deduct(user, "Lucky", deduff);
       return message.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f2 == f3){
       const give4 = eco.give(user,"Lucky", deduff);
       return message.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
       const give5 = eco.give(user,"Lucky", deduff*20);
   ...
   return message.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
           ...
           if ((f1 !== f2) && f2 !== f3){
              const deduct1 = await eco.deduct(user, "Lucky", 50);
                     message.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
           }
           else if ((f1 == f2) && f2 == f3){
              const give1 = await eco.give(user,"Lucky", 100);
                    message.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
           }
           else if ((f1 == f2) && f2 !== f3){
              const give2 = await eco.give(user, "Lucky", 20);
                    message.reply(`${mess3}\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f1 == f3){
              const deduct2 = await eco.deduct(user, "Lucky", 20);
                    message.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f2 == f3){
              const give4 = eco.give(user, "Lucky", 20);
                    message.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
              const give5 = eco.give(user, "Lucky", 1000);
                   message.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
           }
...
async (dest, zk, commandeOptions) => {
  const { zerogroup, sck, mongoschemas, eco, balance1, deduff, deduct1, message, match } = commandeOptions;
  try{
   let zerogroup = (await sck.findOne({ id: message.chat, })) || {};
   let mongoschemas = zerogroup.economy || "false";
   if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
   const kg = 100
           const balance1  = await eco.balance(message.sender, "Lucky")
   ...
   if(lol){
       ...
       const give2 = await eco.give(message.sender, "Lucky", deduff*2);
  ...
  // return await Aviator.bot.sendButtonText(message.chat,str+`You got ${deduff*10} in your wallet.`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, message);
   ...
   const deduct1 = await eco.deduct(message.sender, "Lucky", deduff);
   let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
           let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
